          ...
          while (index >= 0) {
            ...
            if (t().isExportAllDeclaration(d) && d.source) {
              ...
              if (isMatch && ((_opts$exportAllMember = opts.exportAllMembers) === null || _opts$exportAllMember === void 0 ? void 0 : _opts$exportAllMember[d.source.value])) {
                ...
                const init = t().awaitExpression(t().callExpression(t().import(), [t().stringLiteral(`${opts.remoteName}/${getPath(d.source.value, opts.alias || {})}`)]));
                variableDeclarations.unshift(t().variableDeclaration('const', [t().variableDeclarator(id, init)])); // replace node with export const { a, b, c } = __all_exports
                // a, b, c was declared from opts.exportAllMembers

                path.node.body[index] = t().exportNamedDeclaration(t().variableDeclaration('const', [t().variableDeclarator(t().objectPattern(opts.exportAllMembers[d.source.value].map(m => t().objectProperty(t().identifier(m), t().identifier(m)))), t().identifier('__all_exports'))]));
              }
            } // export { bar } from 'foo';
