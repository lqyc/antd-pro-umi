 ...
 */
export declare type ProFieldValueType = 'password' | 'money' | 'textarea' | 'option' | 'date' | 'dateWeek' | 'dateMonth' | 'dateQuarter' | 'dateYear' | 'dateRange' | 'dateTimeRange' | 'dateTime' | 'time' | 'timeRange' | 'text' | 'select' | 'checkbox' | 'rate' | 'radio' | 'radioButton' | 'index' | 'indexBorder' | 'progress' | 'percent' | 'digit' | 'second' | 'avatar' | 'code' | 'switch' | 'fromNow' | 'image' | 'jsonCode';
...
/** 各个组件公共支持的 render */
export declare type ProSchema<T = unknown, Extra = unknown, V = ProSchemaComponentTypes, ValueType = 'text'> = {
    /** @name 确定这个列的唯一值 */
    key?: React.ReactText;
    /**
    ...
    /** 选择如何渲染相应的模式 */
    valueType?: ((entity: T, type: V) => ProSchemaValueType<ValueType>) | ProSchemaValueType<ValueType>;
    /**
     * 支持 ReactNode 和 方法
     *
     * @name 标题
     */
    title?: ((schema: ProSchema<T, Extra>, type: V, dom: React.ReactNode) => React.ReactNode) | React.ReactNode;
     ...
     * @name 自定义编辑模式
     */
    renderFormItem?: (schema: ProSchema<T, Extra> & {
        isEditable?: boolean;
        index?: number;
        type: V;
    }, config: {
    ...
    fieldProps?: ((form: FormInstance<any>, config: ProSchema<T, Extra> & {
        type: V;
        isEditable?: boolean;
        rowKey?: string;
        rowIndex: number;
    ...
    formItemProps?: FormItemProps | ((form: FormInstance<any>, config: ProSchema<T, Extra> & {
        type: V;
        isEditable?: boolean;
        rowKey?: string;
        rowIndex: number;
    ...
    /** @name 映射值的类型 */
    valueEnum?: ((row: T) => ProSchemaValueEnumObj | ProSchemaValueEnumMap) | ProSchemaValueEnumObj | ProSchemaValueEnumMap;
    /** @name 从服务器请求枚举 */
    request?: ProFieldRequestData;
